...
class AutoClicker:
    ...
    def click_at(x, y):
        try:
            ...
            print(f"Error setting cursor position: {error}")

    def toggle_script(self):
        self.running = not self.running
        status_text = "ON" if self.running else "OFF"
        self.logger.log(f'Script toggled {status_text}')
    ...
    def check_and_click_play_button(self, sct, monitor):
        ...
        if current_time - self.last_check_time >= CHECK_INTERVAL:
            ...
            templates = [
                cv2.imread(os.path.join("template", "play_button.png"), cv2.IMREAD_GRAYSCALE),
                cv2.imread(os.path.join("template", "play_button2.png"), cv2.IMREAD_GRAYSCALE),
                cv2.imread(os.path.join("template", "close_button.png"), cv2.IMREAD_GRAYSCALE),
                cv2.imread(os.path.join("template", "captcha.png"), cv2.IMREAD_GRAYSCALE)
            ]
    ...
    def check_and_click_freeze_button(self, sct, monitor):
        freeze_colors_hex = ["#82dce9", "#55ccdc"]  # Add all freeze colors here
        ...
        if current_time - self.last_freeze_check_time >= 1 and current_time >= self.freeze_cooldown_time:
            ...
            for freeze_hsv in freeze_hsvs:
                ...
                for contour in contours:
                    ...
                    self.click_at(cX, cY)
                    self.freeze_cooldown_time = time.time() + 4  # Set a 3 second pause to look for freezes
                    return  # Make only one click
...
if __name__ == "__main__":
    ...
    if not windows:
        print("No windows containing the specified keywords Blum or Telegram.")
        exit()
    ...
    choice = int(input("Enter the number of the window where the Blum bot is open: ")) - 1
    ...
    while True:
        try:
            target_percentage = input("Choose a value between 0 and 1 for random star clicking to collect desired "
                                      "stars.: ")
    ...
    while True:
        try:
            collect_freeze = int(input("Click freezes? 1 - YES, 2 - NO: "))
    ...
    logger = Logger("[https://github.com/alisahinpisirici]")
    logger.log("Welcome to the free Blum game autoclicker script")
    logger.log('After starting the mini-game, press the F6 key on the keyboard')
    target_colors_hex = ["#c9e100", "#bae70e"]
    nearby_colors_hex = ["#abff61", "#87ff27"]
    threshold = 0.8  # Template matching threshold
